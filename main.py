...
import random 
from pygame.locals import * #libreria si alguien teclea
import colores
import objeto #donde estan todos los objetos  del juego
import lugares
import disparo
...
choco = False
helice = False
game = False #tipo booleano
screen_height = 480
width, height = 800, 600 #tamaño y ancho de pixeles
terminar = False #si termina o no el juego
intro = False

# incializar juego
pygame.init()
win = pygame.display.set_mode((width, height))#tupla

#objetos
col = colores.col #colores del juego
avion = objeto.Obj(win, 370, 420, 49, 42, 0, 0, 0)
...
for i in range(n_enemigos):
  enemigos.append(i)
  enemigos[i] = objeto.Obj(win, 0, 0, 0, 0, 0, 0, 0)
...
def reiniciar():
  ...
  for i in range(n_enemigos):
    data_enemigos[i] = [100, i * enemy_box - 480, 42, 30, 6, 1, 0]
...
def hit_color_test(obj, col):
  col = hex(col)
  col = col.lstrip('0x')
  col = tuple(int(col[i:i + int(6 / 3)], 16) for i in range(0, 6, int(6 / 3)))
...
def hit_test():
  ...
  for i in range(n_enemigos):
    if enemigos[i].ty == 5 or enemigos[i].ty == 6 or enemigos[i].ty == 8 or enemigos[i].ty == 9:
        enemigos[i].dir = -1
    else:
        enemigos[i].dir = 1
    ...
    if colisionan(tiro, enemigos[i]) and not enemigos[i].out and tiro.y >= 0:
      enemigos[i].t_expl = 40
      enemigos[i].out = True
      tiro.y = -tiro.h
    
    if colisionan(avion, enemigos[i]) and enemigos[i].ty < 11 and not enemigos[i].out:
      enemigos[i].t_expl = 40
      enemigos[i].out = True
      choco = True
...
def check_enemigos():
    ...
    for i in range(n_enemigos):
      ...
      if game and not intro:
        ...
        if 2 < enemigos[i].ty < 9 and enemigos[i].y > 200 and not enemigos[i].out:
